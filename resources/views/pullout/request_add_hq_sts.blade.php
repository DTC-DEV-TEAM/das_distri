                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select class="form-control select2" style="width: 100%;" required name="pull_out_from" id="pull_out_from">
                                <option value="">-- Select Pullout From --</option>
...
$("#btnSubmit").on('click',function() {
    ...
    var pulloutpull_out_from_var = "";
    var pull_out_deliver_to_var = "";
    $("#pull_out_from").change(function () {
        pulloutpull_out_from_var = $(this).val();

        if(pulloutpull_out_from_var == pull_out_deliver_to_var){
            alert("Same store name is not allowed");
            //$("#pull_out_deliver_to").val("");
            $("#pull_out_from").val("").change();
        }

    });

    $("#pull_out_deliver_to").change(function () {
        pull_out_deliver_to_var = $(this).val();

        if(pulloutpull_out_from_var == pull_out_deliver_to_var){
            alert("Same store name is not allowed");
            //$("#pull_out_deliver_to").val("");
            $("#pull_out_deliver_to").val("").change();
        }
...
$(document).ready(function () {
 $("#excel-form1").submit(function(e){
    ...
    $.ajax({
     url: "{{ route('pullout.upload.excel-stwHQ') }}",
     type: "POST",
     mimeTypes:"multipart/form-data",
     contentType: false,
     cache: false,
...
$(document).ready(function(){
    $(function(){
        ...
        select: function (event, ui) {
            ...
            if(user_channel == "FRANCHISE"){
                if(item_checker == 0){
                    var dcode =  e.stock_code.substring(0, 1);
                    item_restriction.push(dcode); 
                }
                item_checker++;
            }else{
                var dcode =  e.stock_code.substring(0, 1);
