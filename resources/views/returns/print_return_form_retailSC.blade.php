            ...
            <table width="100%">
                    ...
                    @if($row->transaction_type == 3 || $row->transaction_type == 1 )        
                                @if($row->mode_of_return == "STORE DROP-OFF")
                                                        @if(str_contains($row->customer_location, 'SERVICE'))
    ...
    <script type="text/javascript">
        ...
        //var strconfirm = confirm("Are you sure you want to approve this pull-out request?");
            var data = $('#myform').serialize();
                $.ajax({
                        type: 'GET',
                        url: '{{ url('admin/retail_return_diagnosing/FormRejectUpdateStatusRTLSC') }}',
                        data: data,
        ...
        //var strconfirm = confirm("Are you sure you want to approve this pull-out request?");
            var data = $('#myform').serialize();
                $.ajax({
                        type: 'GET',
                        url: '{{ url('admin/retail_return_diagnosing/FormRepairUpdateStatusRTLSC') }}',
                        data: data,
                        success: function( response ){
                            console.log( response );              
