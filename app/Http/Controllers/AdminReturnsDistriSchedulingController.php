	...
	class AdminReturnsDistriSchedulingController extends \crocodicstudio\crudbooster\controllers\CBController {
		...
		public function hook_query_index(&$query) {
			...
			if(CRUDBooster::myPrivilegeName() == "Distri Logistics" || CRUDBooster::myPrivilegeName() == "Logistics"){
				...
				$query->where(function($sub_query) {
					...
					$sub_query->whereIn('returns_status_1', [ReturnsStatus::TO_SCHEDULE_LOGISTICS, ReturnsStatus::RETURN_DELIVERY_DATE])
			...
			if(CRUDBooster::myPrivilegeName() == "Distri Store Ops" || CRUDBooster::myPrivilegeName() == "Distri Ops"){
				...
				$query->where('returns_status_1', ReturnsStatus::TO_SCHEDULE)->where('transaction_type','!=', 2)->orderBy('id', 'asc'); 
				...
				$sub_query->whereIn('returns_status_1', [ReturnsStatus::TO_SCHEDULE_LOGISTICS, ReturnsStatus::TO_SCHEDULE, ReturnsStatus::RETURN_DELIVERY_DATE])
				->where('transaction_type','!=', 2);

				$sub_query->orWhere(function($q) {
					$q->where('returns_status_1', ReturnsStatus::TO_PRINT_PF)
					->whereNotNull('pickup_schedule');
				})->orderBy('returns_header_distribution.id', 'asc')
				->orderBy('returns_status_1', 'asc');
		...
		*/
		...
		public function GetExtractSchedulingReturnsDISTRI(){
			...
			elseif (CRUDBooster::myPrivilegeName() == "Distri Store Ops" || CRUDBooster::myPrivilegeName() == "Distri Ops"){
				
				$result = self::getDistriStoreOpsExportResult($orderData);
				$headers = self::ExportHeader();
			}
		...
		// EXPORT RESULT - STORE OPS
		private function getDistriStoreOpsExportResult($orderData){

			return $orderData->where('returns_status_1', ReturnsStatus::TO_SCHEDULE)->where('transaction_type','!=', 2)->orderBy('returns_header_distribution.id', 'asc'); 
			
		}

		// EXPORT RESULT - OTHER PRIV
		...
		private function ExportRow($orderLine){
			...
			}else
			{
				...
				$scheduled_date = $orderLine->level7_personnel_edited;
