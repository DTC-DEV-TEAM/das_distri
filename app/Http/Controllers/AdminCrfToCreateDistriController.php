...
use App\ModeOfPayment;
	...
	class AdminCrfToCreateDistriController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
	        ...
	        */
			...
			if(CRUDBooster::getCurrentMethod() == 'getIndex' ) {
				...
				"icon"=>"fa fa-download","url"=>CRUDBooster::mainpath('GetExtractCreatedCRFReturnsDISTRI').'?'.urldecode(http_build_query(@$_GET))];
	    ...
	    */
		...
		public function ReturnsCreateEditDISTRI($id)
		{
			$this->cbLoader();
			...
			$data['payments'] = DB::table(env('DB_DATABASE').'.mode_of_payment')
				->where('mode_of_payment','!=','LOYALTY POINTS')
				->where('status','ACTIVE')
				->orderBy('mode_of_refund','asc')
				->groupby('mode_of_refund')
				->get();
		...
		public function GetExtractCreatedCRFReturnsDISTRI() {
				...
				$excel->sheet('orders', function ($sheet) {	
					...
					$to_create_crf = 			ReturnsStatus::where('id','25')->value('id');
						...
						->select(   'returns_header_distribution.*', 
									...
									'closed.name as closed_by',
									'warranty_statuses.*'
									)->whereNotNull('returns_body_item_distribution.category')->where('transaction_type','!=', 2)->where('returns_status_1', $to_create_crf);
