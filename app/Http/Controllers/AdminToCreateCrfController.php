...
use App\ModeOfPayment;
	...
	class AdminToCreateCrfController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
	        ...
	        */
			...
			$this->addaction[] = ['title'=>'Edit','url'=>CRUDBooster::mainpath('ReturnsCreateEditRTL/[id]'),'color'=>'none','icon'=>'fa fa-pencil', "showIf"=>"[returns_status_1] == $to_create_crf"];
	        ...
	        */
			...
			if(CRUDBooster::getCurrentMethod() == 'getIndex' ) {
				...
				"icon"=>"fa fa-download","url"=>CRUDBooster::mainpath('GetExtractCreatedCRFReturnsRTL').'?'.urldecode(http_build_query(@$_GET))];
	    ...
	    */
		...
		public function ReturnsCreateEditRTL($id)
		{
			$this->cbLoader();
			...
			$this->cbView("components.create_crf", $data);
		}


		public function GetExtractCreatedCRFReturnsRTL() {
				...
				$excel->sheet('orders', function ($sheet) {	
					...
					$to_create_crf = 			ReturnsStatus::where('id','25')->value('id');
						...
						->select(   'returns_header_retail.*', 
									...
									'closed.name as closed_by',
									'warranty_statuses.*'
									)->whereNotNull('returns_body_item_retail.category')->where('transaction_type','!=', 2)->where('returns_status_1', $to_create_crf)->groupby('returns_body_item_retail.digits_code');
