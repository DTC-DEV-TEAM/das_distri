	...
	class AdminItemsController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
	        ...
	        */
	        ...
	        {
				$this->button_selected[] = ['label'=>'Set SKU Legend CORE',
											'icon'=>'fa fa-check-circle',
											'name'=>'set_SKU_legend_CORE'];
											
				$this->button_selected[] = ['label'=>'Set SKU Legend NON-CORE',
	    ...
	    */
		...
		public function getItemsUpdatedAPI() {
            ...
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL,"http://dimfs.digitstrading.ph/api/das_items_updated");
            ...
            if(!empty($response["data"])) {
                foreach ($response["data"] as $key => $value) {
    				...
    				try {
    				    $isItemUpdated = DB::table('digits_code')->where('digits_code', $value['digits_code'])->update($value);
    					//$isItemUpdated = Item::where('digits_code', $value['digits_code'])->update($value);
    					DB::commit();
    				} catch (\Exception $e) {
    					DB::rollback();
    				}
