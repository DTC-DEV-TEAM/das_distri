...
use PHPExcel_Style_Fill;
	...
	class AdminRetailReturnCrfController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
	        ...
	        */
			...
			if(CRUDBooster::getCurrentMethod() == 'getIndex' ) {
				...
				"icon"=>"fa fa-download","url"=>CRUDBooster::mainpath('GetExtractCRFReturnsRTL').'?'.urldecode(http_build_query(@$_GET))];
	    ...
	    public function hook_query_index(&$query) {
	        ...
	        if(CRUDBooster::myPrivilegeName() == "Inventory Control"){ 
	            $query->where(function($sub_query){
                    ...
                    $sub_query->Where('returns_status_1', $to_close)->where('transaction_type','!=', 2)->orderBy('id', 'asc');
    				...
    				$sub_query->where('returns_status_1', $to_print_crf)->where('transaction_type','!=', 2)->orderBy('id', 'asc');  
    				$sub_query->orWhere('returns_status_1', $refund_in_process)->where('transaction_type','!=', 2)->orderBy('id', 'asc');
			    });  
	        }
	    ...
	    */
	    public function hook_before_edit(&$postdata,$id) {        
			...
			$data = Input::all();		
			$refunded_date = $data['refunded_date']; 
			//$replced_item = $data['replced_item']; 
			...
			if($ReturnRequest->returns_status_1 == $refund_in_process){
				...
				try {
					...
					$postdata['returns_status'] = 					    $refund_complete;
					$postdata['returns_status_1'] = 					$refund_complete;
					$postdata['refunded_date'] = 						$refunded_date;	
					//$postdata['replced_item'] = 						$replced_item;	
				...
				$date_adjusted 		= $returns_fields['date_adjusted'];
				$stock_adj_ref_no 		= $returns_fields['stock_adj_ref_no'];
				...
				try {
					...
					$postdata['date_adjusted'] = 						$date_adjusted;
					$postdata['stock_adj_ref_no'] = 					$stock_adj_ref_no;
					$postdata['history_status'] = 					    1;
	    ...
	    */
	    public function hook_after_edit($id) {
			...
			if($ReturnRequest->returns_status_1 == $refund_complete){
				...
				$data = ['name'=>$fullname,'status_return'=>$refund_complete,'ref_no'=>$ReturnRequest->return_reference_no,'store_name'=>$ReturnRequest->store];
	    ...
	    */
		...
		public function CRFUpdateStatusRTL(){
			$data = Input::all();		
			...
			if($return_request->returns_status_1 != $to_sor){
				DB::beginTransaction();
				...
				try {
					...
					date('Y-m-d H:i:s')
						...
						ReturnsHeaderRTL::where('id',$request_id)
							...
							'returns_status'=> 			$refund_in_process,
							'returns_status_1'=> 		$to_sor
						]);	
		...
		public function ReturnsClosingEditRTL($id)
		{
			$this->cbLoader();
			if(!CRUDBooster::isUpdate() && $this->global_privilege==FALSE) {    
				CRUDBooster::redirect(CRUDBooster::adminPath(),trans("crudbooster.denied_access"));
			...
			->leftjoin('cms_users as received', 'returns_header_retail.level6_personnel','=', 'received.id')
			->leftjoin('cms_users as closed', 'returns_header_retail.level7_personnel','=', 'closed.id')																		
			->leftjoin('cms_users as createdcrf', 'returns_header_retail.level8_personnel','=', 'createdcrf.id')	
			->select(
			'returns_header_retail.*',
			'scheduled.name as scheduled_by',
			//'tagged.name as tagged_by',	
			...
			'closed.name as closed_by',
			'created.name as created_by',	
			'createdcrf.name as created_crf_by'
			)
			->where('returns_header_retail.id',$id)->first();
		...
		public function GetExtractCRFReturnsRTL() {
				...
				$excel->sheet('orders', function ($sheet) {	
                    ...
                    if(CRUDBooster::myPrivilegeName() == "Inventory Control"){ 
						...
						->select(   'returns_header_retail.*', 
									...
									'closed.name as closed_by',
									'warranty_statuses.*'
						)->whereNotNull('returns_body_item_retail.category')->where('transaction_type','!=', 2)->where('returns_status_1', $to_close)->groupby('returns_body_item_retail.digits_code');
                    }else{
                        $orderData = DB::table('returns_header_retail')
						->leftjoin('warranty_statuses', 'returns_header_retail.returns_status_1','=', 'warranty_statuses.id')
						->leftjoin('cms_users as verified', 'returns_header_retail.level7_personnel','=', 'verified.id')
						...
						->select(   'returns_header_retail.*', 
									...
									'closed.name as closed_by',
									'warranty_statuses.*'
						)->whereNotNull('returns_body_item_retail.category')->where('transaction_type','!=', 2)->where('returns_status_1', $to_print_crf)->groupby('returns_body_item_retail.digits_code')
						->orwhereNotNull('returns_body_item_retail.category')->where('transaction_type','!=', 2)->where('returns_status_1', $refund_in_process)->groupby('returns_body_item_retail.digits_code')
						;
                    }
		...
		public function ReturnsClosingEditReplaceRTL($id)
		{
			$this->cbLoader();
			if(!CRUDBooster::isUpdate() && $this->global_privilege==FALSE) {    
				CRUDBooster::redirect(CRUDBooster::adminPath(),trans("crudbooster.denied_access"));
			...
			$data['store_list'] = Stores::where('channels_id',$channels->id)->get();
				...
				$this->cbView("returns.edit_closing_retail_acc", $data);
