...
use App\StoresFrontEnd;
	...
	class AdminReturnsToCloseController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function hook_query_index(&$query) {
	        ...
	        if(CRUDBooster::myPrivilegeName() == "Store Ops"){
	            ...
	            $query->where(function($sub_query){
					...
					$sub_query->Where('returns_status_1', $to_ship_back)->where('transaction_type','!=', 2)->whereIn('returns_header.stores_id', $storeList)->orderBy('returns_status_1', 'asc');
	            ...
	            if(CRUDBooster::myPrivilegeName() == "Service Center"){
    	            ...
    	            $query->where(function($sub_query){
    					...
    					//$sub_query->where('returns_status_1', $to_schedule_aftersales)->orderBy('returns_status_1', 'asc');  
    					$sub_query->Where('returns_status_1', $to_ship_back)->where('transaction_type','!=', 2)->orderBy('returns_status_1', 'asc');
    					$sub_query->orWhere('returns_status_1', $for_replacement)->where('transaction_type','!=', 2)->orderBy('returns_status_1', 'asc'); 
    					//$sub_query->Where('returns_status_1', $for_replacement)->where('transaction_type','!=', 2)->orderBy('returns_status_1', 'asc');
    					//$sub_query->orWhere('returns_status_1', $requested)->orderBy('returns_status_1', 'asc');
    				});
	    ...
	    */
	    public function hook_before_edit(&$postdata,$id) {        
	        //Your code here
						...
						}else if($ReturnRequest->diagnose == "REPLACE"){
	    ...
	    */
	    public function hook_after_edit($id) {
			...
			}else if($ReturnRequest->returns_status_1 == $replacement_complete){
	    ...
	    */
	    ...
	    public function ReturnsDetailClose($id)
		{
			$this->cbLoader();
			if(!CRUDBooster::isUpdate() && $this->global_privilege==FALSE) {    
				CRUDBooster::redirect(CRUDBooster::adminPath(),trans("crudbooster.denied_access"));
		...
		public function GetExtractReturnsScheduling(){
			...
			Excel::create($filename, function ($excel) {
				$excel->sheet('orders', function ($sheet) {	
					...
					if(CRUDBooster::myPrivilegeName() == "Logistics"){ 
					...
					}elseif(CRUDBooster::myPrivilegeName() == "Aftersales" || CRUDBooster::myPrivilegeName() == "Retail Ops" || CRUDBooster::myPrivilegeName() == "Ecomm Ops"){ 
						...
						->select(   'returns_header.*', 
									...
									'closed.name as closed_by',
									'warranty_statuses.*'
						)->whereNotNull('returns_body_item.category')->where('transaction_type','!=', 2)->where('returns_status_1', $to_ship_back)->groupby('returns_body_item.digits_code');
						...
						->select(   'returns_header.*', 
									...
									'closed.name as closed_by',
									'warranty_statuses.*'
						)->whereNotNull('returns_body_item.category')->where('transaction_type','!=', 2)->where('returns_status_1', $to_ship_back)->whereIn('returns_header.stores_id', $storeList)->groupby('returns_body_item.digits_code');
						...
						->select(   'returns_header.*', 
									...
									'closed.name as closed_by',
									'warranty_statuses.*'
						)->whereNotNull('returns_body_item.category')->where('returns_status_1', $pending)->whereIn('returns_header.stores_id', $storeList)->groupby('returns_body_item.digits_code')
						->orwhereNotNull('returns_body_item.category')->where('returns_status_1', $requested)->whereIn('returns_header.stores_id', $storeList)->whereIn('returns_header.stores_id', $storeList)->whereIn('returns_header.stores_id', $storeList)->whereIn('returns_header.stores_id', $storeList)->groupby('returns_body_item.digits_code')
						->orwhereNotNull('returns_body_item.category')->where('returns_status_1', $to_ship_back)->whereIn('returns_header.stores_id', $storeList)->whereIn('returns_header.stores_id', $storeList)->whereIn('returns_header.stores_id', $storeList)->groupby('returns_body_item.digits_code')
						->orwhereNotNull('returns_body_item.category')->where('returns_status_1', $to_schedule_aftersales)->whereIn('returns_header.stores_id', $storeList)->whereIn('returns_header.stores_id', $storeList)->groupby('returns_body_item.digits_code')
						->orwhereNotNull('returns_body_item.category')->where('returns_status_1', $to_schedule_logistics)->whereIn('returns_header.stores_id', $storeList)->groupby('returns_body_item.digits_code');		
	    ...
	    public function ReturnsClosingEdit($id)
		{
			$this->cbLoader();
			if(!CRUDBooster::isUpdate() && $this->global_privilege==FALSE) {    
				CRUDBooster::redirect(CRUDBooster::adminPath(),trans("crudbooster.denied_access"));
