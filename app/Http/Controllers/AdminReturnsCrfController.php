...
use PHPExcel_Style_Fill;
	...
	class AdminReturnsCrfController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
	        ...
	        */
			...
			if(CRUDBooster::getCurrentMethod() == 'getIndex' ) {
				...
				"icon"=>"fa fa-download","url"=>CRUDBooster::mainpath('GetExtractReturnsCRF').'?'.urldecode(http_build_query(@$_GET))];
	    ...
	    */
	    public function hook_after_edit($id) {
			...
			if($ReturnRequest->returns_status_1 == $refund_complete){
				...
				$data = ['name'=>$fullname,'status_return'=>$refund_in_process,'ref_no'=>$ReturnRequest->return_reference_no,'store_name'=>$ReturnRequest->store];
	    ...
	    */
		...
		public function CRFUpdateStatus(){
			$data = Input::all();		
		...
		public function GetExtractReturnsCRF() {
				...
				$excel->sheet('orders', function ($sheet) {	
					...
					$sheet->setColumnFormat(array(
						...
						->select(   'returns_header.*', 
									...
									'warranty_statuses.*'
						)->whereNotNull('returns_body_item.category')->where('transaction_type','!=', 2)->where('returns_status_1', $to_print_crf)->groupby('returns_body_item.digits_code')
						->orwhereNotNull('returns_body_item.category')->where('transaction_type','!=', 2)->where('returns_status_1', $refund_in_process)->groupby('returns_body_item.digits_code')
						;
