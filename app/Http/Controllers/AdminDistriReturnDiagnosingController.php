...
use PHPExcel_Style_Fill;
	...
	class AdminDistriReturnDiagnosingController extends \crocodicstudio\crudbooster\controllers\CBController {
		...
		//	$this->form[] = ['label'=>'Verified Items Included','name'=>'verified_items_included','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
	        ...
	        */
			...
			if(CRUDBooster::getCurrentMethod() == 'getIndex' && (CRUDBooster::myPrivilegeName()=="Service Center")){
				...
				"icon"=>"fa fa-download","url"=>CRUDBooster::mainpath('GetExtractDiagnosingReturnsDISTRISC').'?'.urldecode(http_build_query(@$_GET))];
			}else{
				...
				$this->index_button[] = ["title"=>"Export Returns",
				"label"=>"Export Returns",
				"icon"=>"fa fa-download","url"=>CRUDBooster::mainpath('GetExtractDiagnosingReturnsDISTRI').'?'.urldecode(http_build_query(@$_GET))];
	    ...
	    public function hook_query_index(&$query) {
			...
			if (CRUDBooster::myPrivilegeName() == "Tech Lead") {
				...
				$query->where(function($sub_query){
					...
					$sub_query->whereIn('returns_status_1', [$to_for_action, $to_print_return_form])->orderBy('id', 'desc');
				});
			}else {
			...
			// 	$sub_query->where('returns_status_1', $to_diagnose)->orderBy('id', 'asc');  
			// 	$sub_query->orwhere('returns_status_1', $to_print_return_form)->orderBy('id', 'asc');  
	    ...
	    */
	    public function hook_before_edit(&$postdata,$id) {        
			...
			if($ReturnRequest->returns_status_1 == $to_assign_inc ) {
				...
				$store_id =     StoresFrontEnd::where('store_name', $ReturnRequest->store_dropoff )->where('channels_id', 7 )->first();
				...
				if ($field_1 == 'Save' && CRUDBooster::myPrivilegeName() == "RMA Specialist") {
					...
					return redirect()->action('AdminDistriReturnDiagnosingController@ReturnsReturnFormPrintDISTRI',['id'=>$ReturnRequest->id])->send();
				}
	    ...
	    */
		...
		public function ReturnsDiagnosingDISTRIEdit($id)
		{
			$this->cbLoader();
			...
			$data['row'] = ReturnsHeaderDISTRI::
				...
				->leftjoin('cms_users as rma', 'returns_header_distribution.received_by_rma_sc','=', 'rma.id')
				->leftjoin('cms_users as turnover_by', 'returns_header_distribution.rma_receiver_id','=', 'turnover_by.id')	
				->select(
			...
			$this->cbView("components.distribution.to_diagnose", $data);
		...
		public function ReturnsReturnFormPrintDISTRI($id)
		{
			...
			$this->cbView("returns.print_return_form_distri", $data);
		...
		public function FormRejectUpdateStatusDISTRI(){
			$data = Input::all();		
		...
		public function FormRejectUpdateStatusDISTRISC(){
		...
		public function FormRepairUpdateStatusDISTRISC(){
		...
		public function GetExtractDiagnosingReturnsDISTRI() {
				...
				$excel->sheet('orders', function ($sheet) {	
					...
					if(CRUDBooster::myPrivilegeName() == "Tech Lead"){
						...
						->select( 	'returns_header_distribution.*', 
									...
									'warranty_statuses.*'
									)->whereNotNull('returns_body_item_distribution.category')->where('transaction_type', 0)->where('returns_status_1', $to_diagnose)->where('level2_personnel', CRUDBooster::myId())
									->orwhereNotNull('returns_body_item_distribution.category')->where('transaction_type', 0)->where('returns_status_1', $to_assign_inc)
									->orwhereNotNull('returns_body_item_distribution.category')->where('transaction_type', 0)->where('returns_status_1', $to_for_action)
									->orwhereNotNull('returns_body_item_distribution.category')->where('transaction_type', 0)->where('returns_status_1', $to_print_return_form);
		...
		public function GetExtractDiagnosingReturnsDISTRISC() {
			...
			Excel::create($filename, function ($excel) {
				$excel->sheet('orders', function ($sheet) {	
					...
					->select( 	'returns_header_distribution.*', 
								...
								'closed.name as closed_by',
								'warranty_statuses.*'
								)->whereNull('returns_body_item_distribution.category')->where('transaction_type', 1)->where('returns_status_1', $requested)->whereIn('returns_header_distribution.stores_id', $storeList)
						//->orwhereNotNull('returns_body_item.category')->where('transaction_type', 0)->where('returns_status_1', $to_receive_sor)
						->orwhereNull('returns_body_item_distribution.category')->where('transaction_type', 1)->where('returns_status_1', $to_print_return_form)->whereIn('returns_header_distribution.stores_id', $storeList);
		...
		public function TechLeadDISTRI($id)
		{
			$this->cbLoader();
			if(!CRUDBooster::isUpdate() && $this->global_privilege==FALSE) {    
				CRUDBooster::redirect(CRUDBooster::adminPath(),trans("crudbooster.denied_access"));
