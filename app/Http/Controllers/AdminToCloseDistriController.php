...
use PHPExcel_Style_Fill;
	...
	class AdminToCloseDistriController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
	        ...
	        */
			...
			if(CRUDBooster::getCurrentMethod() == 'getIndex' ) {
				...
				"icon"=>"fa fa-download","url"=>CRUDBooster::mainpath('GetExtractClosingReturnsDISTRI').'?'.urldecode(http_build_query(@$_GET))];
		...
		*/
		...
		public function ReturnsClosingEditDISTRIOPS($id)
		{
			$this->cbLoader();
			...
			if(CRUDBooster::myPrivilegeName() == "SDM"){
				...
				$this->cbView("returns.edit_closing_distri_ops", $data);
			}
			
		}


		public function GetExtractClosingReturnsDISTRI() {
				...
				$excel->sheet('orders', function ($sheet) {	
					...
					if(CRUDBooster::myPrivilegeName() == "SDM"){
						...
						->select(   'returns_header_distribution.*', 
									...
									'warranty_statuses.*'
						)->whereNotNull('returns_body_item_distribution.category')->where('transaction_type','!=', 2)->where('returns_status_1', $for_replacement_sdm);
						...
						->select(   'returns_header_distribution.*', 
									...
									'closed.name as closed_by',
									'warranty_statuses.*'
						)->whereNotNull('returns_body_item_distribution.category')->where('transaction_type','!=', 2)->where('returns_status_1', $to_ship_back)->whereIn('returns_header_distribution.stores_id', $storeList)
						->orwhereNotNull('returns_body_item_distribution.category')->where('transaction_type','!=', 2)->where('returns_status_1', $for_replacement)->whereIn('returns_header_distribution.stores_id', $storeList);
						...
						->select(   'returns_header_distribution.*', 
									...
									'closed.name as closed_by',
									'warranty_statuses.*'
						)->whereNotNull('returns_body_item_distribution.category')->where('transaction_type','!=', 2)->where('returns_status_1', $to_ship_back)
						->orwhereNotNull('returns_body_item_distribution.category')->where('transaction_type','!=', 2)->where('returns_status_1', $for_replacement);
					}
