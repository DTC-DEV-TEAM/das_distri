	    ...
	    */
	    public function hook_query_index(&$query) {
			...
			$query->where(function($sub_query){
				...
				$sub_query->where('returns_status_1', $to_schedule)->where('transaction_type','!=', 2)->orderBy('returns_status_1', 'asc');  
				...
				$excel->sheet('orders', function ($sheet) {	
					...
					$pending = ReturnsStatus::where('id','19')->value('id');
						...
						->select(   'returns_header_retail.*', 
									...
									'closed.name as closed_by',
									'warranty_statuses.*'
									)->whereNotNull('returns_body_item_retail.category')->where('transaction_type','!=', 2)->where('returns_status_1', $to_schedule)->groupby('returns_body_item_retail.digits_code')
									->orwhereNotNull('returns_body_item_retail.category')->where('transaction_type','!=', 2)->where('returns_status_1', $pending)->groupby('returns_body_item_retail.digits_code');
