        ...
        <tr class="active">           
          ...
          ?>   
            ...
            @if(CRUDBooster::isUpdate() || CRUDBooster::isDelete() || CRUDBooster::isRead())                     
                <th width='{{$button_action_width?:"auto"}}' style="text-align: center; vertical-align: top;">{{trans("crudbooster.action_label")}}</th>
            @endif                   
          @endif
...
<div class="col-md-8 col-sm-8 col-xs-12">{!! urldecode(str_replace("/?","?",$result->appends(Request::all())->render())) !!}</div>
<?php
...
<span class="pull-right total-rows">{{ trans("crudbooster.filter_rows_total") }} : {{ $from }} {{ trans("crudbooster.filter_rows_to") }} {{ $to }} {{ trans("crudbooster.filter_rows_of") }} {{ $total }}</span>
</div>
            ...
            <div class='row-filter-combo row'>
              ...
              <div class='col-sm-3'>
                <select name='filter_column[{{$col["field_with"]}}][type]' data-type='{{$col["type_data"]}}' class="filter-combo form-control">
                  ...
                  <option {{ (CRUDBooster::getTypeFilter($col["field_with"]) == 'empty')?"selected":"" }} value='empty'>Is Empty (Blank)</option>
                </select>
              </div><!--END COL_SM_4-->
      ...
      {!! CRUDBooster::getUrlParameters() !!}
        <div class="modal-body">                    
          <div class="form-group">
            ...
            <input type='text' name='filename' class='form-control' required value='Export {{ $module->name }} - {{date("d M Y - h.i.sa")}}'/>
          ...
          <div class="form-group">
            ...
            <input type='number' name='limit' class='form-control' required value='100000' max="100000" min="1" /> 
            <div class='help-block'>{{trans("crudbooster.export_dialog_help_maxdata")}}</div>         
          </div> 

          <div class="form-group">
            <label>Select All / Unselect All</label>
            <div class='checkbox'>
              <label>
              <input type='checkbox' id='select_columns' value='' checked="true" />  
              </label>    
            </div> 
          </div> 

          <div class='form-group'>
            <label>{{trans("crudbooster.export_dialog_columns")}}</label>
            <?php 
              $counter=0;
            ?>
            @foreach($columns as $col)
          ...
          <div class="form-group">
            ...
            <select name='fileformat' class='form-control'>
              ...
              <option value='xls'>Microsoft Excel5 (XLS)</option>
              <option value='xlsx'>Microsoft Excel2007 (XLSX)</option>
              <!--
