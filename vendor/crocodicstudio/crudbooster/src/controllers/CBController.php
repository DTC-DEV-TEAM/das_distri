...
class CBController extends Controller
{
    ...
    public function input_assignment($id = null)
    {
        ...
        foreach ($this->data_inputan as $ro) {
            ...
            //custom parent select check-box
            // if ($ro['type'] == 'check-box') {
            //     if ($ro['datatable']) {
            //         if ($inputdata == '') {
            //             $this->arr[$name] = 0;
            //         }
    ...
    public function postAddSave()
    {
        ...
        try {
	        ...
	        if($error_code == '1062'){
	            $message = trans("crudbooster.alert_duplicate",['duplicate_data'=>$error_msg->errorInfo[2]]);
				return redirect()->back()->with(['message_type'=>'danger','message'=>$message]);
	        }
	        else{
	        	$message = trans("crudbooster.alert_database_error",['database_error'=>$error_msg->errorInfo[2]]);
				return redirect()->back()->with(['message_type'=>'danger','message'=>$message]);
	        }
	        DB::rollback();
		    /* Transaction failed. */
