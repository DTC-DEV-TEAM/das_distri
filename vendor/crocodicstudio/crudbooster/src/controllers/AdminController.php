...
class AdminController extends CBController
{
    ...
    public function postLogin()
    {
        ...
        if (\Hash::check($password, $users->password)) {
            ...
            if($priv->name == "Retail Ops" || $priv->name == "Service Center" || $priv->name == "Store Ops") {
                //$approvalMatrix = ApprovalMatrix::where('approval_matrices.cms_users_id',  $users->id)->first();
                //$storeList = '(' . $approvalMatrix->store_list . ')';
                $approvalMatrix =     DB::table("cms_users")->where('cms_users.id', $users->id)->get();  //ApprovalMatrix::where('approval_matrices.cms_users_id', $users->id)->get(); 
				...
				foreach($approvalMatrix as $matrix){
				    $count++;
				    if($count==1) {
				        $storeList_string = $matrix->stores_id;
				    }
				    else {
				        $storeList_string .= ','.$matrix->stores_id;
				    }
